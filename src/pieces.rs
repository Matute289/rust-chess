...
fn create_pieces(
    ...
    for (rank_idx, rank_str) in fen_board.split('/').enumerate() {
        // FEN rank index 0 = rank 8 (top) = piece.x 7; index 7 = rank 1 = piece.x 0
        let piece_x = 7u8.saturating_sub(rank_idx as u8);
        let mut file: u8 = 0;
        ...
        for ch in rank_str.chars() {
            ...
            let maybe = match ch.to_ascii_lowercase() {
                'k' => Some((PieceType::King,   vec![king_handle.clone(), king_cross.clone()], vec![Vec3::new(-0.2, 0., -1.9); 2])),
                'q' => Some((PieceType::Queen,  vec![queen_handle.clone()],                    vec![Vec3::new(-0.2, 0., -0.95)])),
                'r' => Some((PieceType::Rook,   vec![rook_handle.clone()],                     vec![Vec3::new(-0.1, 0., 1.8)])),
                'b' => Some((PieceType::Bishop, vec![bishop.clone()],                           vec![Vec3::new(-0.1, 0., 0.)])),
                'n' => Some((PieceType::Knight, vec![knight_1.clone(), knight_2.clone()],       vec![Vec3::new(-0.2, 0., 0.9); 2])),
                'p' => Some((PieceType::Pawn,   vec![pawn_handle.clone()],                      vec![Vec3::new(-0.2, 0., 2.6)])),
                _   => None,
            };
            if let Some((pt, meshes, offsets)) = maybe {
                let color = if ch.is_uppercase() { PieceColor::White } else { PieceColor::Black };
                let mat   = if color == PieceColor::White { white.clone() } else { black.clone() };
                spawn_piece(&mut commands, mat, color, pt, meshes, offsets, s, (piece_x, file));
            }
            file += 1;
        }
    }
